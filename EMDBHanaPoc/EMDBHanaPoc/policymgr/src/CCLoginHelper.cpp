    ...
    if (res.result() == http::status::ok)
	{
       ...
       auto& body = res.body();
        ...
        size_t nPosForm = body.find("<form");
            ...
            //get auth url
            {
                const std::string str_key = "action=\"";
                size_t nPosAction = body.find(str_key, nPosForm);
                size_t nPosEnd = body.find("\"", nPosAction + str_key.length());
                strLoginSubmitPath = body.substr(nPosAction + str_key.length(), nPosEnd - nPosAction - str_key.length());
            }

            //get lt
            {
                const std::string str_key = "name=\"lt\"";
                size_t nPosLtName = body.find(str_key, nPosForm);
                size_t nltBegin = body.find("\"", nPosLtName + str_key.length());
            ...
            //get execute
            {
                const std::string str_key = "name=\"execution\"";
                size_t nPosExecName = body.find(str_key, nPosForm);
                size_t nExecBegin = body.find("\"", nPosExecName + str_key.length());
